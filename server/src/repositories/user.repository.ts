...
export const createUser = async (payload: IUserPayload, session: Session & Partial<SessionData> & SessionData): Promise<User> => {
...
export const login = async (payload: IUserPayload, session: Session & Partial<SessionData> & SessionData): Promise<User> => {
  ...
  if(!(await argon2.verify(user.password, payload.password))) throw new Error("Incorrect Username or Password") // incorrect password
